// pages/carResource/bookkeeping.js
...
create(store, {
  ...
  formSubmit(e) {
    ...
    this.addBill(this.data.formData).then(res => {
      this.downGood({
        goods_id: this.data.options.id
      }).then(res => {
        wx.showToast({
  ...
  onLoad(options) {
    ...
    this.getGoodDetail({
      ...
      this.setData({
        carDetail: res.data,
        'formData.goods_id': options.id
      })
    })

    commonStore.bind('bookkeepingPage', this)
    commonStore.init()
